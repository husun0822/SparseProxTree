...
tree = build_tree(data = data,depth = 3,iter = 100)
plot(tree)
...
alldata = list(output1,output2,output3,output4)
acc = lapply(alldata,tree_eval)


# run the sparse proximity tree multiple times
acc.history = list(0,0,0,0)

for (i in 1:50){
  acc = lapply(alldata,tree_eval)
  for (j in 1:4){
    acc.history[[j]] = acc.history[[j]] + acc[[j]]
  }
}

for (j in 1:4){
  acc.history[[j]] = acc.history[[j]]/50
}
